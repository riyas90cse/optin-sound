...
'geolocation':{},
'insite_trigger':'on',
'trigger_elements':'#id,.class,element',
'play_condition':'onload',
'exclude_pages':{},
'exit_intent_pop':'on',
'widget':{'tbt':'Nishant Sharma', 'tbc':'#ff0000', 'tbb':'#ffffff','brandicon':'http://convertsound.local/images/convertsound-fav.png','ht':'Big offer for early birds', 'hc':'#ff0000', 'hb':'#ffffff', 'dt':'Tap below to reveal the offer for you!!!', 'dc':'#2239c8', 'db':'#dccece', 'position':'bottom-right', 'icon':'http://convertsound.local/images/convertsound-fav-def.gif'},
'opw':{'optinM':'optin messsage','sp_optinM':'special optin messsage','thankyou':'Congratulations!!! You are enrolled in luckydraw now. Thankyou for your submitting your entry. We will communicate to the winners!'},
'url':'http://convertsound.local/optin',
'uid':'2',
'cid':'3'
}
...
document.getElementById('optinform').addEventListener('submit', function(evt){
    evt.preventDefault();
    alert('0');
    returnToPreviousPage(evt);
...
var data = {};
    data.name=document.querySelector('#optinform input[name="name"]').value;
    data.email=document.querySelector('#optinform input[name="email"]').value;
    data.phone= document.querySelector('#optinform input[name="phone"]').value;
    data.cid= convertsound.cid;
    data.uid=convertsound.uid;
  ...
  // convertsound.postData(convertsound.url,json,'callback');
  // if invalid
  if (e.preventDefault) {
    e.preventDefault();
  } else {
...
document.getElementById('ops-chat-full-bg').addEventListener('click', function() {
  removeExitPopup();
},false);
...
function showExitPopup(){
    ...
    element.classList.add('exitpopup');
    var element1 = document.getElementById('ops-chat-full-bg');
...
function removeExitPopup(){
    removePopup();
    var element = document.getElementById('ops-chat');
    element.classList.remove('exitpopup');
    var element1 = document.getElementById('ops-chat-full-bg');
...
convertsound.postData= function  (url,strdata,callback){
...
document.addEventListener('mousemove', function(e) {
  ...
  if (isLeaving <= 50 && cs.exit_intent_pop=='on' && exipop) {
    showExitPopup();
    exipop=false;
  }
});
