					...
					<form enctype="multipart/form-data" method="PUT" id="form-sound-post">  
					    ...
					    <div class="modal-body">
							...
							<div class="form-group" id="customuploadpreview" style="display:none;">
								<h4 class="modal-title">Sound Preview:</h4>
								<img src="<?php echo e(url('/images/convertsound-fav.png')); ?>" id="customsoundplay" alt="play/pause" title="To Play Click Here">							
								<audio class="form-control" name="custom_soundpreview" id="custom_soundpreview" controls>
									<source src="<?php echo e(url('/uploads/sound/')); ?>" type="audio/mpeg">
										Your browser does not support the audio tag.
								</audio>
...
jQuery('#form-sound-post').submit(function(event){
	...
	$.ajax({
	    ...
	    success:function(response) {
	    	$('#custom_soundpreview').attr('src',"<?php echo e(url('/')); ?>"+'/'+response);
	    	$('#customuploadpreview').slideDown(300);
	    	$('#custom-upload-sound').val("<?php echo e(url('/')); ?>"+'/'+response);
        	// $('#selectimageupload').modal('toggle');
	    }
	});
