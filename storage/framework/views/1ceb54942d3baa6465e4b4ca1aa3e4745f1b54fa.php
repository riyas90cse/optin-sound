				...
				<div class="row" id="action_section">					
					<div class="col-md-10 col-xs-10 col-sm-10 col-md-offset-1 pt5" style="display:block;">
							...
							<div class="form-group">
								...
								<div class="col-md-12">
									<input type="text" class="form-control" name="overlay_siteurl" id="overlay_siteurl" required="required" placeholder="Write Overlay Name"/>	
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-md-12">
									<span class="custom_url_part1" id=""><?php echo e(url('/')); ?></span><input type="text" class="custom_url_part2" name="handle" id="handle" required="required" placeholder="Write Unique Handle Name"/>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-md-12">
									<input type="text" class="form-control" name="meta_title" id="meta_title" required="required" placeholder="Write Meta Title"/>	
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-12">
									<input type="text"  class="form-control" name="meta_description" id="meta_description" required="required" placeholder="Write Meta Description"/>
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-md-12">
									<select class="form-control selectpicker" id="campaign_id" name="campaign_id">
										...
										<option  value="<?php echo e($value->id); ?>"><?php echo e($value->campaign_name); ?></option>
			...
			<div class="row" id="variation_section" style="display:block;">
				<div class="col-md-12">
				    <div class="modal-header">
						...
						<p class="opt-title center">Overlay Link Created Successfully!</p>
				    </div>
				    <div class="modal-body">
						<p class="opt-title">Overlay Campaign Link<span class="tooltip1" title="Copy Below link and past in browser for overlay campaign preview!"><i class="fa fa-info-circle"></i></span></p>
						<p class="center">
...
jQuery('#widget_action1').submit(function(e){
	...
	if(ajaxchk3==true){
		ajaxchk3=false;
		$.ajax({
			url: "<?php echo e(url('/overlay/ajaxsave')); ?>",
			type: "GET",
			processData: false,
			data:formdata+'&domain_name='+domain_name+'&domain_id='+domain_id+'&campaign_name='+campaign_name,
			success: function(data){
				var ol='<?php echo e(url("/")); ?>/'+$('input[name="handle"]').val();
				$('.overlaylink').html(ol);
				$('.overlaylink').attr('href',ol);
				$('.copybutton').data('copy',ol);
				$('.shareCampaign').each(function(){
					$(this).data('sharelink',ol);
					$(this).data('handle',$('input[name="handle"]').val());
				});
